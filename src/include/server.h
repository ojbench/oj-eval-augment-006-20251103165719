...
int mine_count[35][35];    // Number of adjacent mines for each grid
...
void VisitBlock(int r, int c) {
  ...
  // If mine count is 0, recursively visit all neighbors (BFS)
  if (mine_count[r][c] == 0) {
    ...
    while (!q.empty()) {
      ...
      for (int k = 0; k < 8; k++) {
        ...
        if (nr >= 0 && nr < rows && nc >= 0 && nc < columns &&
            !is_visited[nr][nc] && !is_marked[nr][nc] && !is_mine[nr][nc]) {
...
void AutoExplore(int r, int c) {
  ...
  if (marked_count == mine_count[r][c]) {
    for (int k = 0; k < 8; k++) {
      ...
      int nc = c + dc[k];
      if (nr >= 0 && nr < rows && nc >= 0 && nc < columns &&
          !is_visited[nr][nc] && !is_marked[nr][nc]) {
        VisitBlock(nr, nc);
        if (game_state != 0) {
          return;
        }
...
void ExitGame() {
  ...
  if (game_state == 1) {
    // If win, all mines are considered marked
    marked_mine_count = total_mines;
  } else {
    // If lose, count only correctly marked mines
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < columns; j++) {
